...
export const useStudio = create<StudioState>((set, get) => ({
  ...
  onNodesChange: (changes) => {
    ...
    for (const c of changes) {
      if (c.type === "remove") clearPoll(c.id);
    }
    get().scheduleSave();
  },
  ...
  generateImage: async (nodeId) => {
    ...
    get().updateNode(nodeId, { status: "running", progress: 0, error: undefined, startedAt: Date.now(), jobIds: [] });
    ...
    get().updateNode(nodeId, { jobIds: jobs.map((j) => j.id) });
    pollImageNode(nodeId, jobs.map((j) => j.id), set, get);
  },
    ...
    try {
      ...
      if (file && Array.isArray(file.boards) && file.boards.length) {
        ...
        for (const b of file.boards) {
          boardsData[b.id] = {
            nodes: (b.nodes as AppNode[]) ?? [],
            edges: (b.edges as Edge[]) ?? [],
            counters: b.counters ?? {},
          };
        }
...
function pollImageNode(nodeId: string, jobIds: string[], _set: SetFn, get: GetFn) {
  clearPoll(nodeId);
